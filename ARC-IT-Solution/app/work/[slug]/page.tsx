  ...
  return (
    <>
      ...
      <main id="main-content" role="main">
        ...
        {/* Full-width UI preview */}
        {Preview && (
          <section aria-label="Project preview" style={{ background: '#080806', borderBottom: '1px solid var(--border-subtle)', overflow: 'hidden' }}>
            <div style={{ maxWidth: '100%', aspectRatio: '16/8', position: 'relative' }}>
              <Preview />
            </div>
          </section>
        )}
        ...
        <section aria-label="Project outcomes" style={{ background: 'var(--bg-secondary)', borderBottom: '1px solid var(--border-subtle)' }}>
          <div className="container-site">
            <div style={{ display: 'grid', gridTemplateColumns: `repeat(${project.metrics.length},1fr)`, gap: '0' }} className="metrics-grid">
      ...
      <Footer />
      <style>{`
        @media(max-width:900px){.case-body-grid{grid-template-columns:1fr !important;}}
        @media(max-width:600px){.metrics-grid{grid-template-columns:1fr 1fr !important;}}
      `}</style>
    </>
  )
}
